  ...
  try {
      ...
      joblis.each((index,item)=>{
          ...
          try{
              ...
              infos.push(infobox)
              console.log(title,address,salary,type,edureq,yearreq,company,jobid,iutype,"----",index)
...
async function getinfosFromPage(callback,maxPn,category,delay=1666) {
  ...
  for(let i=1;i<=maxPn;i++){
      ...
      console.log(category + '-标签-' + maxPn + '-页数-'+ i +"  "+ new Date().toString())
      if(callback){
        callback(infos)
      }
  }
